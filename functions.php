<?php
    ...
    function learn_land_files() {
        ...
        //note that we prevent caching of css by the 4th argument "microtime()"
        wp_enqueue_style('learn_land_main_styles', get_stylesheet_uri(), null, microtime() );
        ...
        //note: changing the 4th argument to "microtime()" prevents js caching during development
        wp_enqueue_script('main_page_js', get_theme_file_uri('/js/scripts-bundled.js'), null, microtime(), true);
    };
    ...
    //below, add new "action" that creates appropriate tab titles
    function learn_land_features() {
        //we want to enable a feature for a theme and so we call the function and pass in the feature we want to enable
        add_theme_support('title-tag');
    }

    add_action('after_setup_theme', learn_land_features);
